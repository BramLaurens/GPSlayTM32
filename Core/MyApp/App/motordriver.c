...
{	
	...
	{
		PWM_Set(PWM_FREQUENCY, abs(speedLeft), &htim4, TIM_CHANNEL_4); // A1
		PWM_Set(PWM_FREQUENCY, 0, 			   &htim4, TIM_CHANNEL_1); // A2
	}
	else
	{
		PWM_Set(PWM_FREQUENCY, 0,			   &htim4, TIM_CHANNEL_4); // A1
		PWM_Set(PWM_FREQUENCY, abs(speedLeft), &htim4, TIM_CHANNEL_1); // A2
	}

	if (directionRight == FORWARD)
	{
		PWM_Set(PWM_FREQUENCY, abs(speedRight), &htim12, TIM_CHANNEL_2); // B1
		PWM_Set(PWM_FREQUENCY, 0,	 			&htim12, TIM_CHANNEL_1); // B2
	}
	else
	{
		PWM_Set(PWM_FREQUENCY, 0,				&htim12, TIM_CHANNEL_2); // B1
		PWM_Set(PWM_FREQUENCY, abs(speedRight), &htim12, TIM_CHANNEL_1); // B2
	}
}
